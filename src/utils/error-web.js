//TODO: Убрать из native зависимость с expo если возможно и сделать один файла для Веба и Native
const ERROR_CODES = {
...
export default (code, type) => {
    try{
        let namespace = 'errors'
        if (type==="SUCCESS")  {
            namespace = "alerts"
        }
        let translated_message = i18n.t(namespace+':'+code);
        console.log("Message", translated_message);
        if( translated_message && translated_message.length && translated_message!==code ){
